...
from lib.dataset.cocoeval.customeval import CustomEval
...
from lib.utils.keypoints_constants import keypoint_names, part_mapping
...
class COCODataset(JointsDataset):
    ...
    def __init__(self, cfg, root, image_set, is_train, transform=None):
        ...
        if cfg.DATASET.TEST_LABEL_PATH != "":
            self.label_path = cfg.DATASET.TEST_LABEL_PATH
        else:
        ...
        self.per_keypoint_eval = False
        if not is_train:
            self.per_keypoint_eval = cfg.TEST.PER_KEYPOINT_EVAL

        self.pretrained_model = "bulk"
        if not is_train:
            self.pretrained_model = cfg.TEST.MODEL_FILE
    ...
    def _get_ann_file_keypoint(self):
        ...
        if self.label_path:
            return os.path.join(self.root, 'annotations', self.label_path)

        prefix = 'person_keypoints' \
            if 'test' not in self.image_set else 'image_info'
        annotation = self.image_set.split("/")[0]

        return os.path.join(
            self.root,
            'annotations',
            prefix + '_' + annotation + '.json'
        )
    ...
    def image_path_from_index(self, index):
        ...
        prefix = 'train2017' if 'train' in self.image_set else prefix
        prefix = 'val2017' if 'val' in self.image_set else prefix
    ...
    def evaluate(self, cfg, preds, output_dir, all_boxes, img_path, keypoint_accs,
                 *args, **kwargs):
        rank = cfg.RANK
        ...
        if 'test' not in self.image_set:
            info_str = self._do_python_keypoint_eval(res_file, keypoint_accs, cfg)
    ...
    def _do_python_keypoint_eval(self, res_file, keypoint_accs, cfg):
        ...
        if self.per_keypoint_eval:
            for i, keypoint in enumerate(keypoint_names["coco"]):
                ...
                info_str.append(("PCK@0.5", keypoint_accs[i + 1].avg))
        ...
        info_str.append(("PCK@0.5", keypoint_accs[0].avg))
    ...
    def _get_occluded(self, cfg):
        ...
        name = ""
        width = 0
        if cfg.FORCED_OCCLUSION != "":
            name = "vanila"
            return cfg.FORCED_OCCLUSION, name, 0
        occlusion_transforms = list(filter(lambda a: isinstance(a, ImageTransform), self.transform.transforms))
        if not self.is_train and len(occlusion_transforms) > 0:
    ...
    def _create_results_save_folder(self):
        pretrained_model = os.path.basename(os.path.dirname(self.pretrained_model))
